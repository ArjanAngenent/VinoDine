...
#################### PACKAGE ACTIONS ###################
reinstall_package:
    @pip uninstall -y VinoDine || :
    @pip install -e .

run_preprocessor:
    python -c 'from VinoDine.interface.main import preprocessor; preprocessor()'

#run_train:
#    python -c 'from VinoDine.interface.main import train; train()'

run_model:
    python -c 'from VinoDine.interface.main import model; model()'

#run_evaluate:
#    python -c 'from VinoDine.interface.main import evaluate; evaluate()'

run_all: run_preprocessor run_model

#run_workflow:
#    PREFECT__LOGGING__LEVEL=${PREFECT_LOG_LEVEL} python -m VinoDine.interface.workflow

run_api:
    uvicorn VinoDine.api.fast:app --reload
